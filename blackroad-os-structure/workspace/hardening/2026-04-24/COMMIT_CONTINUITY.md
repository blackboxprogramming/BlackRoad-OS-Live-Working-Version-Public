...
- 2026-04-24 — Expanded root-domain site registry and unified live routes (home + API) for the requested root domains.
  - Change:
    - Expanded `/Applications/BlackRoadOSInc/cloudflare-worker-home/src/blackroad/sites.ts` to include the full root-domain set (incl. `blackroadinc.us`, `blackroadquantum.*`, `aliceqi.com`, `lucidia.*`).
    - Extended `/Applications/BlackRoadOSInc/cloudflare-worker-home/src/blackroad/siteRegistry.ts` API view to include `primaryCTA`.
    - Updated `/Applications/BlackRoadOSInc/cloudflare-worker-home/public/home/index.html` to apply domain branding via `/api/sites` (registry-backed), with a safe fallback map.
    - Attached Worker routes for **both** `/<*>` and `/api/*` across the root domains list using `wrangler deploy --routes ...`.
  - Verified:
    - `https://blackroad.io/api/health` returns `x-blackroad-rev: 2026-04-24a`
    - `https://blackroad.io/api/sites` returns 21 domains and includes `primaryCTA` keys
    - `https://blackroadquantum.info/api/sites` and `https://lucidia.earth/api/sites` return JSON (21) (no HTML fallback)
    - `https://blackroad.io/home/` now includes “DOMAIN-AWARE BRANDING (Registry-backed)” code path
  - Still blocked:
    - Headless browser visual QA remains blocked by Cloudflare WAF (see `BR-VIS-001`)
    - Local screenshot helper fails due to Swift toolchain mismatch (see `BR-VIS-003`)
  - Visual QA:
    - A Playwright screenshot attempt captured the Cloudflare block page:
      - `/Applications/BlackRoad/BlackRoadOS/workspace/hardening/2026-04-24/screenshots/blackroad.io-home-playwright.png`

- 2026-04-24 — Fixed RoadTrip E2E by routing the product domain to the shared registry-backed Worker.
  - Change:
    - Updated `/Applications/BlackRoadOSInc/cloudflare-worker-home/src/index.ts` to treat **product host roots** as their product surface (`/` → product `route`).
    - Attached `roadtrip.blackroad.io/`, `roadtrip.blackroad.io/*`, and `roadtrip.blackroad.io/api/*` to `blackroad-grayscale` Worker routes.
  - Verified:
    - `https://roadtrip.blackroad.io/` → `302 /home/apps/RoadTrip/` → `200` (`x-blackroad-worker: home`)
    - `https://roadtrip.blackroad.io/api/health` returns `x-blackroad-rev: 2026-04-24a`
    - `https://roadtrip.blackroad.io/api/products` returns JSON (no D1 error)
  - Still blocked:
    - Visual QA is still blocked for headless tools by Cloudflare WAF (see `BR-VIS-001`)
